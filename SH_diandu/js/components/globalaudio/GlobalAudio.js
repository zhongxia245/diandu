...
function GlobalAudio(config) {
	config = config || {};
	this.callback = config.callback;
	var data = config.data

	this.selector = '__globalAudioSetting__'
	this.id = data.id;
	this.name = data.name || "";
	this.src = data.src || "";
...
GlobalAudio.prototype.render = function () {
	...
	div.setAttribute('id', that.selector)
	$('body').append(div)
	...
	layer.open({
		...
		cancel: function (index, layero) {
			var data = that.data
			window.global_audio_util.set(data.id, data)
			layer.close(index)
			return false;
		}
	})
...
GlobalAudio.prototype.renderPageItem = function (data, pageIndex) {
	...
	for (var i = 0, length = data.length; i < length; i++) {
		...
		if (!obj.isRemove) {
			...
			if (i === pageIndex - 1) {
				_time = '00:00'
				this.data.pageConfig[i] = _time
			}
			_time = this.data.pageConfig[i] || _time
			...
			html.push('  <div class="ga-content-page-item-time">' + (_time) + '</div>')
			html.push('</div>')
		}
	}
	return html.join(' ');
...
GlobalAudio.prototype.bindEvent = function () {
   ...
   */
	that.$container.on('click', '.ga-content-page-item-time', function (e) {
		...
		that.data.pageConfig[id] = that.formatTime(audio.currentTime)
	...
	that.$container.on('change', '.ga-content-page-item-time', function (e) {
		...
		that.data.pageConfig[id] = timeStr
	})

...
/**
 * 自定义点读点设置
 * TODO: 把逻辑和功能单独分开，不都写在一起(点模式，视频模式，音频模式，3D模型模式，考试模式，超链接模式，注解模式等)
 */
window.CustomPointSetting = (function () {
     ...
     */
		...
		this.render(function (html) {
			...
			that.initVar(); 				//初始化变量
			that.showTabByType(); 	//根据点读点类型，展示选项卡按钮
			that.bindEvent(); 			//绑定事件
			that.initData(); 				//初始化数据
		})
	}
	 ...
	 * 根据点读点类型，判断是否展示选项卡
	 */
	CustomPointSetting.prototype.showTabByType = function () {
		var type = this.data.pointData.type
		switch (type) {
			case 'video':
				this.$tabVideo.show()
				this.$tabPoint.show()
   ...
   * 渲染页面
   */
	CustomPointSetting.prototype.render = function (callback) {
		//计算图片在自定义视频区域上的大小
		...
		// 渲染handlebars的数据
		var renderData = {
			pointTypeClass: this.pointTypeClasses[this.data.title.type],

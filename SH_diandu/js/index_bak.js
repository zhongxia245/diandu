...
var _edit = (function () {
    ...
    function _initPointByData(pageIndex, w, h, point) {
        ...
        //初始化视频,音频,图文的数据
        _initPointItemData(dataid, type, fileName, url);
    }
     ...
     * @private
     */
    function _initPointItemData(dataid, type, fileName, url) {
        switch (type) {
            case "1":
                console.log(dataid, "video fileName", fileName, 'url', url)
                break;
            case "2":
                console.log(dataid, "audio fileName", fileName, 'url', url)
                break;
            case "3":
                console.log(dataid, "imgText fileName", fileName, 'url', url)
                break;
        }
    }
...
function handleSubmit(e) {
    ...
    Model.addDianduPage(data, function (result) {
        alert('保存成功！' + result);
        alert('创建点读页成功,id=' + result + ',点击确定返回单元列表!');

...
var AUTOPLAYINTERVAL = 20 * 1000;  //自动播放时间
...
function styleHandler() {
    var $size_switch = $('#size_switch');
    var $size_progressAll = $('#size_progressAll');
    var $size_time = $('#size_time');
    var $size_progress = $('#size_progress');
    if (isVertical) {
        $size_switch.css({
            width: '100%'
        });
        $size_progressAll.css({
            width: '100%'
        })
        $size_time.css({
            width: '100%',
...
function initSwipe() {
    ...
    //上面设置默认会自动播放,这边暂停自动播放先
    galleryThumbs.stopAutoplay();

    $("#cbk_auto").not("[data-switch-no-init]").bootstrapSwitch({
        onSwitchChange: function (e, state) {
            event.preventDefault();
            if (state) {
                galleryThumbs.params.autoplay = AUTOPLAYINTERVAL;
    ...
    var $alert = $('.intervalTime');
    var $p = $(".progress");
    $p.slider({max: 100, min: 10, now: 20});
    $p.on("sliderchange", function (e, result) {
        AUTOPLAYINTERVAL = result.value * 1000;
        var text = "(" + result.value + "秒)";
        $alert.text(text);
    });
}
...
function initCircle(data, w, h) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var top = parseFloat(data[i].y) * h * SCALEH;
...
function bindEvent() {
    ...
    /*上下滑动,展示缩略图和自动播放控制轴*/
    //向上滑动
    $('body').on('swipeUp', function (ev) {
        ev.preventDefault();
        console.log("show")
        $(".gallery-main").css('opacity', 1).slideDown(300);
    });

    //向下滑动
    $('body').on('swipeDown', function (ev) {
        ev.preventDefault();
        console.log("hide")
        $(".gallery-main").slideUp(300);
    });

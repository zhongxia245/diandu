...
<div class="main">
    ...
    <section class="baseinfo">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="intro" rows="5" placeholder="介绍测试的相关内容、注意点等,简洁不能超过100字"><?php echo $video['saytext'];?></textarea>
        </div>
        <div class="form-group">
            <label>
                缩略图
        ...
        <div class="setting-bigimg">
            ...
            <div style="{{style}}" class="setting-bigimg-img id_bg" id="id_bg{{index}}" data-index="{{index}}">
                ...
                <div class="setting-bigimg-tip-h bigimg-tip">
                    <h3>提示</h3>
                    <p style="font-size:16px">手机横屏比例为16:9，对应像素通常为1920*1080。建议上传大于此分辨率的图片作为背景图</p>
                    <p style="font-size:16px;margin-top:10px">选择图片时,可批量上传,按CTRL键加选,按shift链选(第一个和最后一个之间的所有,可以按ctrl+a
                        选择一个文件夹下的所有图片)
                    <p/>
                </div>
                <div class="setting-bigimg-tip-v bigimg-tip">
                    <h3>提示</h3>
                    <p style="font-size:16px">手机竖屏比例为9:16，对应像素通常为1080*1920。建议上传大于此分辨率的图片作为背景图</p>
                    <p style="font-size:16px;margin-top:10px">选择图片时,可批量上传,按CTRL键加选,按shift链选(第一个和最后一个之间的所有,可以按ctrl+a
                        选择一个文件夹下的所有图片)
                    <p/>
                </div>
...
<script src="js/class/ImgText.js"></script>
<script>
    var teamid=<?php echo $teamid;?>;
    var unitid=<?php echo $unitid;?>;
    var cover_list=[];
    $("#ulImgs").sortable({ stop: function(event, ui) {upload_cover_sort();} });
    ...
    function upload_cover_callback(pic_url)
    {
        ...
        $ode.click(function(){upload_cover_del(this);return false;});
